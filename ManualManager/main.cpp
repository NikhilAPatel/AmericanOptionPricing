...
std::vector<double> runSimulation(double S0, double sigma, double r, double dt, double D, int N, double KP, int NSim, int simThreadCount){
    ...
    #pragma omp parallel
    {
        while (simulationsCompleted<NSim){
            if(omp_get_thread_num() < simThreadCount){
...
int main(int argc, char* argv[]){
    ...
    for(auto atc: tc){
        ...
        std::vector<double> result = runSimulation(S0, sigma, r, dt, D, N, KP, NSim, atc);
...
//        cout<<price<<endl;
        cout<<"("<<atc<<", "<<((price-1.110)/1.110)*100.0<<", "<<duration.count()<<"),"<<endl;
    }



    cout<<"ManualManager"<<endl;
//    cout<< price << endl;
//    cout<<((price-1.110)/1.110)*100.0<<endl;
